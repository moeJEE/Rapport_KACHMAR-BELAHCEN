...
% Inclus depuis 11_appendices.tex, Annexe A
% Les diagrammes non retenus restent en place mais sont masqués via
% \iffalse...\fi (jamais supprimés) pour respecter la contrainte de
% 100 pages totales du rapport.
% =====================================================================

% Macro fiche UC sans numérotation (usage principal dans le texte)
\providecommand{\ucfichephase}[7]{%
  \begin{tcolorbox}[
...
La modélisation complète des cas d'utilisation comporte neuf diagrammes~: un diagramme de \textbf{configuration initiale du projet} (création et paramétrage), puis un diagramme par phase du cycle de migration. Par souci de concision, seuls les diagrammes à plus fort impact sur le projet sont présentés ci-après~: la \textbf{configuration initiale d'un projet}, la \textbf{phase~5 --- Transform} et la \textbf{phase~8 --- Cutover}, chacun accompagné des fiches descriptives de ses cas d'utilisation les plus représentatifs.
...
% -----------------------------------------------------------------------
\iffalse % Diagramme masqué (hors sélection à fort impact) — conservé pour réutilisation future
...
La plateforme ADMP~AI~Studio comporte cinq diagrammes de séquence modélisant ses flux dynamiques les plus critiques. Par souci de concision, seuls les deux diagrammes à plus fort impact transverse sont présentés ci-après~: la \textbf{génération des packages de migration} et la \textbf{réconciliation source-cible}.
...
\subsubsection*{Génération des packages}
...
\begin{umlseqdiag}
  ...
  \begin{umlcall}[op={generatePackage(projectId\, format)}, return=bundleZip]{designer}{web}
    \begin{umlcall}[op={POST /api/codegen/compile-project/\{projectId\}}, return=zip]{web}{compiler}
      \begin{umlcall}[op={loadDocuments(RC\, DP\, TT\, DS)}, return=docs]{compiler}{storage}
      \end{umlcall}
      \begin{umlcall}[op={generateArtifacts(docs\, format)}, return=files]{compiler}{compiler}
      \end{umlcall}
      \begin{umlcall}[op={assembleBundle(files\, format)}, return=bundlePath]{compiler}{storage}
      \end{umlcall}
    \end{umlcall}
  \end{umlcall}
\end{umlseqdiag}
\end{tikzpicture}
\caption{Génération des packages (vue plateforme, multi-format)}
\label{fig:sd02}
\end{figure}
...
\subsubsection*{Réconciliation source-cible}
...
\end{umlseqdiag}
\end{tikzpicture}
\caption{Réconciliation source-cible}
\label{fig:sd04}
\end{figure}
...
Le diagramme de classes (Figure~\ref{fig:class}) modélise les quatorze entités principales de la plateforme ADMP~AI~Studio persistées dans la base de données PostgreSQL.
...
\begin{tikzpicture}
  ...
  %% Databricks CI/CD run (generated by bundle pipeline, reported back via API)
  \umlclass[x=16, y=-8]{DatabricksBundleRun}{
    +id : UUID \\
    +jobName : String \\
  ...
  \umlcompo[mult1=1, mult2=*]{Project}{DatabricksBundleRun}
\end{tikzpicture}%
}% end scalebox
