...
% ---------------------------------------------------------------------
\subsubsection{Philosophie et principes fondateurs}
\label{subsubsec:admp-philosophie}

L'\textbf{Accenture Data Migration Platform (ADMP)} est une plateforme propriétaire développée par Accenture depuis plus de 25 ans, dédiée à l'accélération, l'automatisation et la sécurisation des projets de migration de données à grande échelle. Sa philosophie repose sur un constat fondamental : \og~\textit{Migration is not an investment priority: it is a cost that delivers few benefits in itself. However, Migration is highly risky and an essential part of the wider transformation project success}~\fg{}.

Pour répondre à ce constat, ADMP s'articule autour de trois principes fondateurs :

\begin{itemize}[leftmargin=*]
    \item \textbf{Smart Designs} : des documents structurés (Word/Excel) qui décrivent les règles de transformation, les mappings de champs, les relations entre tables et les contraintes métier. Ces documents sont à la fois lisibles par les équipes fonctionnelles et interprétables automatiquement par la plateforme.
    
    \item \textbf{No-Code / Low-Code} : \og~\textit{No ETL developer required. Smart designs are executed on ADMP platform with no manual action required}~\fg{}, ce qui élimine le fossé Design-Build identifié comme limitation majeure des solutions existantes.
    
    \item \textbf{Transparence totale} : \og~\textit{No black box effect}~\fg{}, les équipes fonctionnelles ont une visibilité et un contrôle complets sur le processus de migration, contrairement aux outils ETL traditionnels.
\end{itemize}

L'ADMP fait partie de l'\textbf{Accenture Migration Suite}, un ensemble de composants de migration qui peuvent être utilisés individuellement ou combinés pour transformer, nettoyer, archiver, réactiver et rechercher des données, offrant ainsi une réponse complète à la décommission des systèmes legacy.

% ---------------------------------------------------------------------
\subsubsection{Architecture fonctionnelle existante}
\label{subsubsec:admp-architecture}

L'architecture actuelle d'ADMP se compose de quatre modules principaux, décrits dans le Tableau~\ref{tab:admp-modules}.

\begin{table}[H]
\centering
\caption{Modules de la plateforme ADMP}
\label{tab:admp-modules}
\begin{tabularx}{\textwidth}{llX}
\toprule
\textbf{Module} & \textbf{Type} & \textbf{Fonction} \\
\midrule
...
L'hébergement de la plateforme est flexible : \textbf{SaaS} (datacenter sécurisé) ou \textbf{On-Premise} (installé dans l'environnement du client). Les transferts de données sont sécurisés via SFTP/VPN.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{img/architecture_ADMP.png}
    \caption{Architecture fonctionnelle de la plateforme ADMP}
    \label{fig:admp-architecture}
\end{figure}

% ---------------------------------------------------------------------
\subsubsection{Méthodologie ADM for ADMP}
\label{subsubsec:adm-admp}

La méthodologie \textbf{ADM for ADMP} (Accenture Delivery Method for ADMP) structure le processus de migration en cinq phases :

\begin{enumerate}[leftmargin=*]
    \item \textbf{Discovery} : Analyse initiale du périmètre, identification des systèmes source et cible, cadrage du projet.
    
    \item \textbf{Data Analysis (Design Phase 1)} : Phase itérative comprenant la création des Data Scopes (SDS, TDS), du Table Mapping et du Data Mapping. L'objectif est de \og~\textit{validate the scope of the Data Migration and identify most of the gaps between the source system and the target system}~\fg{}.
    
    \item \textbf{Detailed Design (Design Phase 2)} : Élaboration des spécifications fines : Migration Steps (MS), Data Structure (DS), Intermediary Data Scope (IDS), Record Creation (RC), Data Population (DP), et Transcodification Tables (TT).
    
    \item \textbf{Build \& Test} : Le Core Engine génère automatiquement les programmes de migration à partir des Smart Designs. Les données sont extraites, transformées et chargées, puis validées via des rapports automatiques.
    
    \item \textbf{Deploy} : Migration finale en production, réconciliation des volumes, et validation post-migration.
\end{enumerate}

L'ensemble de la méthodologie est enseigné dans la \textbf{Core ADMP Design School (CIDS)}, une formation intensive de \textbf{deux semaines (80 heures)} couvrant la théorie, la pratique et les exercices sur tous les livrables ADMP.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{img/adm_cycle.png}
    \caption{Les cinq phases de la méthodologie ADM for ADMP}
    \label{fig:adm-cycle}
\end{figure}

% ---------------------------------------------------------------------
\subsubsection{Les livrables structurés}
\label{subsubsec:livrables}

Chaque livrable ADMP répond à un besoin précis dans la chaîne de migration, comme le résume le Tableau~\ref{tab:admp-livrables}.

\begin{table}[H]
\centering
\caption{Livrables structurés de la plateforme ADMP}
\label{tab:admp-livrables}
\begin{tabularx}{\textwidth}{llX}
\toprule
\textbf{Livrable} & \textbf{Phase} & \textbf{Rôle} \\
\midrule
SDS & Data Analysis & Dictionnaire des tables et champs sources \\
TDS & Data Analysis & Dictionnaire des tables et champs cibles \\
Table Mapping & Data Analysis & Correspondance tables source $\leftrightarrow$ cible \\
Data Mapping & Data Analysis & Correspondance champs source $\leftrightarrow$ cible \\
...
% ---------------------------------------------------------------------
\subsubsection{Bilan : forces et chiffres clés}
\label{subsubsec:admp-bilan}

Les chiffres clés d'ADMP ont déjà été présentés dans le Tableau~\ref{tab:admp-kpi} de l'introduction générale. La comparaison directe entre l'approche ADMP et l'approche ETL standard révèle des avantages structurels significatifs, synthétisés dans le Tableau~\ref{tab:admp-vs-etl}.

\begin{table}[H]
\centering
\caption{Comparaison ADMP vs ETL standard}
\label{tab:admp-vs-etl}
\begin{tabularx}{\textwidth}{lXX}
\toprule
\textbf{Critère} & \textbf{ETL Standard} & \textbf{ADMP Smart Designs} \\
\midrule
